...
=======|__/=========================
  API References of myTestorProxy
           ----- oOo -----
[procedure] api_testor_cell_not_contains
====================================
...
procedure mytestorproxy.api_testor_cell_not_contains ( 
...
execute stmt_testor_test_cells;
deallocate prepare stmt_testor_test_cells;
call mytestorproxy.api_testor_cell_not_contains( @v_token, @v_test_id, @v_suite_id, @v_case_id, 'str.not_contains.1', @v_table, @v_field, @v_where, @v_order, @v_limit, @v_str_b ); select @v_test_id as `test_id`, @v_suite_id as `suite_id`, @v_case_id as `case_id`;
...
| case        | test               | message                                                                                               |
+-------------+--------------------+-------------------------------------------------------------------------------------------------------+
| table-users | str.not_contains.1 | [str.not_contains.1] test (cell_not_contains) is success. \nOperand: Hello world!\nValue: community\n |
+-------------+--------------------+-------------------------------------------------------------------------------------------------------+
