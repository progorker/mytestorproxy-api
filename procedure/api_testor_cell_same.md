...
=======|__/=========================
  API References of myTestorProxy
           ----- oOo -----
  [procedure] api_testor_cell_same
====================================
...
procedure mytestorproxy.api_testor_cell_same ( 
...
execute stmt_testor_test_cells;
deallocate prepare stmt_testor_test_cells;
call mytestorproxy.api_testor_cell_same( @v_token, @v_test_id, @v_suite_id, @v_case_id, 'str.same.1', @v_table, @v_field, @v_where, @v_order, @v_limit, @v_str_b ); select @v_test_id as `test_id`, @v_suite_id as `suite_id`, @v_case_id as `case_id`;
...
| case        | test       | message                                                                                  |
+-------------+------------+------------------------------------------------------------------------------------------+
| table-users | str.same.1 | [str.same.1] test (cell_same) is success. \nOperand: Hello world!\nValue: Hello world!\n |
+-------------+------------+------------------------------------------------------------------------------------------+
